...
void WriteKernelInformation( int iTargetFd, int iTotalKernelSectorCount, int iKernel32SectorCount , int kernel64_sector_count, int uapp_sector_count);
...
int main(int argc, char* argv[])
{
	...
	int uapp_sector_count = 0;
    int iSourceSize;

    // 커맨드 라인 옵션 검사
    if( argc < 4 )
    {
        fprintf( stderr, "[ERROR] imageMaker kernel32.bin kernel64.bin uapp.elf\n" );
        exit( -1 );
    }
    ...
    printf( "[INFO] %s size = [%d] and sector count = [%d]\n", argv[ 2 ], iSourceSize, iKernel64SectorCount );
    ...
    printf( "[INFO] %s size = [%d] and sector count = [%d]\n",
                argv[ 3 ], iSourceSize, uthread_sector_count );
#endif

    //--------------------------------------------------------------------------
    // Copy application ELF file to the disk image
    //--------------------------------------------------------------------------
    printf("[INFO] Copying user apps to image file\n");
    if( ( iSourceFd = open( argv[3], O_RDONLY ) ) == -1 )
    {
     fprintf( stderr, "[ERROR] %s open fail\n", argv[ 3 ] );
     exit( -1 );
    ...
    uapp_sector_count += AdjustInSectorSize( iTargetFd, iSourceSize );
    printf( "[INFO] %s size = [%d] and sector count = [%d]\n", argv[ 3 ], iSourceSize, uapp_sector_count );
    ...
    WriteKernelInformation( iTargetFd, iKernel32SectorCount + iKernel64SectorCount + uapp_sector_count, iKernel32SectorCount, iKernel64SectorCount, uapp_sector_count);
...
void WriteKernelInformation( int iTargetFd, int iTotalKernelSectorCount, int iKernel32SectorCount, int kernel64_sector_count, int uapp_sector_count )
{
    ...
    lPosition = lseek( iTargetFd, 0x83, SEEK_SET ); // 0x83: 프로텍티드모드 이미지에서 TOTAL_COUNT의 주소
    if( lPosition == -1 )
    {
    ...
    write( iTargetFd, &usData, 2 );
    usData = ( unsigned short ) uapp_sector_count;
    write( iTargetFd, &usData, 2 );
    ...
    printf( "[INFO] Total sector count of user thread [%d]\n",
        iTotalKernelSectorCount - iKernel32SectorCount - kernel64_sector_count );
}
